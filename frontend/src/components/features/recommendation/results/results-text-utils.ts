...
const DOMAIN_AXIS_PRIORITY: Record<string, string[]> = {
  ...
  plate: ["푹신한 바닥감", "묵직한 타격감", "또렷한 고음", "차분한 감쇠음", "플렉스"],
  foam: ["차분한 감쇠음", "깊은 저음", "푹신한 바닥감", "울림"],
  layout: ["타이핑", "밀도", "컴팩트", "배열", "묵직한 타격감", "차분한 감쇠음"],
  case: ["차분한 감쇠음", "깊은 저음", "묵직한", "울림", "소리"],
  keycap: ["차분한 감쇠음", "또렷한 고음", "매끈한 타건감", "소리"],
};
...
function buildEvidenceFeelHook(
  ...
  if (d === "switch") {
    if (/택타일|갈축/.test(specText)) return "촉각이 살아 있는 택타일 스위치예요";
    if (/리니어/i.test(specText)) return "부드럽게 쳐지는 리니어 스위치예요";
