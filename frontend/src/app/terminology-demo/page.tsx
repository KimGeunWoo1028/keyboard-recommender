    ...
    <div className="bg-ca-surface-container-low">
      <PageShell className="max-w-4xl space-y-8">
        ...
        <ManusSurfaceCard className="space-y-4" padding="lg">
          <h2 className="font-headline text-lg font-bold">1) 기본 리졸버 (`defaultDictionaryResolver`)</h2>
          ...
          <ul className="space-y-3">
            {SAMPLES.map((phrase) => {
              ...
              return (
                <li key={phrase} className="rounded-xl border border-border bg-ca-surface-container-low p-4">
                  <p className="mb-2 text-sm font-semibold text-ca-on-surface">&quot;{phrase}&quot;</p>
                  <pre className="max-h-80 overflow-auto rounded-lg border border-border bg-white p-3 font-mono text-xs leading-relaxed text-ca-on-surface-variant dark:bg-ca-surface-container">
        ...
        <ManusSurfaceCard className="space-y-4" padding="lg">
          <h2 className="font-headline text-lg font-bold">2) 여러 의미 블렌드 (`sensePick: blend`)</h2>
          <p className="text-sm text-ca-on-surface-variant">
            <code className="rounded-md border border-border bg-ca-surface-container-low px-1 text-xs">marbly</code>
          ...
          {(["highestConfidence", "blend"] as const).map((strategy) => {
            ...
            return (
              <div key={strategy} className="rounded-xl border border-border bg-ca-surface-container-low p-4">
                <p className="mb-2 text-sm font-semibold">marbly — sensePick: {strategy}</p>
                <pre className="max-h-64 overflow-auto rounded-lg border border-border bg-white p-3 font-mono text-xs text-ca-on-surface-variant dark:bg-ca-surface-container">
        ...
        <ManusSurfaceCard className="space-y-3" padding="lg">
          <h2 className="font-headline text-lg font-bold">3) 단일 용어 (`termToTraitDelta`)</h2>
          <p className="text-sm text-ca-on-surface-variant">드롭다운·태그 한 개만 넣을 때 쓰는 API입니다.</p>
          <pre className="rounded-xl border border-border bg-ca-surface-container-low p-4 font-mono text-xs text-ca-on-surface-variant">
