#!/usr/bin/env python3
"""Merge scraped plate/foam compatibility specs into swagkey seed metadata."""
...
from keyboard_recommender.catalog.spec_enrichment import merge_component_specs_into_seed


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--seed", required=True, help="Path to swagkey_products.seed.json")
    parser.add_argument("--specs", required=True, help="Path to plate/foam specs JSON")
    ...
    merged = merge_component_specs_into_seed(seed_payload, specs_payload)
