"""Keyboard layout catalog + per-trait scores."""
...
class KeyboardLayout(Base, TimestampMixin):
    ...
    trait_scores: Mapped[list["KeyboardLayoutTraitScore"]] = relationship(
        "KeyboardLayoutTraitScore",
        back_populates="keyboard_layout",
        cascade="all, delete-orphan",
        passive_deletes=True,
    )


class KeyboardLayoutTraitScore(Base):
    __tablename__ = "keyboard_layout_trait_scores"

    keyboard_layout_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("keyboard_layouts.id", ondelete="CASCADE"),
        primary_key=True,
    )
    ...
    keyboard_layout: Mapped[KeyboardLayout] = relationship(back_populates="trait_scores")
    trait: Mapped["RecommendationTrait"] = relationship(
        "RecommendationTrait",
        back_populates="keyboard_layout_scores",
    )
