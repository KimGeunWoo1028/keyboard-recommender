"""Keyboard case catalog + per-trait scores (mirrors other component families)."""
...
class KeyboardCase(Base, TimestampMixin):
    __tablename__ = "keyboard_cases"
    ...
    trait_scores: Mapped[list["KeyboardCaseTraitScore"]] = relationship(
        "KeyboardCaseTraitScore",
        back_populates="keyboard_case",
        cascade="all, delete-orphan",
        passive_deletes=True,
    )


class KeyboardCaseTraitScore(Base):
    __tablename__ = "keyboard_case_trait_scores"

    keyboard_case_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("keyboard_cases.id", ondelete="CASCADE"),
        primary_key=True,
    )
    ...
    keyboard_case: Mapped[KeyboardCase] = relationship(back_populates="trait_scores")
    trait: Mapped["RecommendationTrait"] = relationship(
        "RecommendationTrait",
        back_populates="keyboard_case_scores",
    )
